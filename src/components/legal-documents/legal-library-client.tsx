...
export function LegalLibraryClient({ documents, dataSource }: Props) {
  ...
  const typeCounts = useMemo(() => countDocumentsByType(documents), [documents]);
  const fieldCounts = useMemo(() => countDocumentsByField(documents), [documents]);

  const results = useMemo(
    () => filterLegalDocuments(documents, appliedFilters),
  ...
  return (
    <>
      <PageBanner
        ...
        subtitle="Tìm kiếm theo số hiệu, trích yếu, loại văn bản, cơ quan ban hành, lĩnh vực và tình trạng hiệu lực — đồng bộ từ vbpl.vn qua Supabase."
        image="insights"
      />
      ...
      <section className="section-premium bg-ivory">
        <div className="container-premium space-y-8">
          {dataSource === "supabase" && (
            <p className="rounded-sm border border-emerald-500/20 bg-emerald-500/5 px-4 py-2 text-sm text-emerald-800">
              Dữ liệu đang lấy từ Supabase (đồng bộ AI từ vbpl.vn).
            </p>
          )}
